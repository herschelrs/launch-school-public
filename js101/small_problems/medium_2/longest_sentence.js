function longestSentence(str) {
  let sentences = str.split(".").map(x => x.split("!")).flat().map(x => x.split("?")).flat().map(x => x.trim());
  let sentenceLengths = sentences.map(sentence => sentence.split(" ")).map(x => x.length);
  //console.log(sentenceLengths)
  return Math.max(...sentenceLengths);
}
